...
<label style="color: black;
    font-size: 15px;
    font-weight: bold; margin-top:20px;text-align:center; width:100%;" class="detai">Stamping Receipt</label>
	 <label style="color: black;
    font-size: 12px;
    font-weight: bold;margin:2% 0; " class="detai">Request Details:</label>
		 <tr>
			<td >
			...
			<td >
                Request Time : <?php echo $requested_time; ?>
            </td>
			<td >
               Request Date : <?php if(isset($requested_date) && $requested_date!=""){echo date("d-m-Y", strtotime($requested_date));}?>
            </td>
			<td >Serial No : <?php foreach($list7 as $key1){ if(isset($key1->serial_no)){ echo $key1->serial_no; } }?></td>
        </tr>
		<tr>
			<td >Customer Name : <?php echo $customer_name; ?></td>
			<td colspan=2>Address  : <?php echo $address.' '.$address1; ?> </td>
            <td colspan=2>Mobile : <?php echo $mobile; ?></td>		
        </tr>
			<tr>
		...
		<tr>
            <td colspan=5>
                Location : <?php foreach($list7 as $key1){ if(isset($key1->service_loc)){ echo $key1->service_loc; } }?>
            </td>
...
<label style="color: black;
    font-size: 12px;
    font-weight: bold; margin:2% 0;" class="detai">Stamping Details:</label>
	 		<table class="hh"style="margin:5px 0px; width:100%;" >
		<?php foreach($stamping_details As $stkey){
			...
			?>
		<tr>
		<td>Year : <?php echo $stkey->year; ?></td><td>Quarter : <?php echo $stkey->quarter; ?></td><td colspan=2>Kg : <?php echo $stkey->kg; ?></td>
		</tr>
			<tr>
		<td colspan=2>Received : <?php if(in_array('vcplate', $stamping_received)){echo "VC Plate"; } if(in_array('vcpaper', $stamping_received)){echo " VC Paper"; } if(in_array('bothplatepaper', $stamping_received)){echo " VC Plate + Paper"; } ?></td><td>Stamping Charge : <?php echo $stkey->stamping_charge; ?></td><td>Penalty : <?php echo $stkey->penalty; ?></td>
		</tr>
			
		<tr>
		 <td colspan=4>
                <div class="pull-right"><b>Total : <?php echo $stkey->stamping_charge + $stkey->penalty; ?></b></div>
            </td>
		</tr>
		<?php } ?>
		</table>
...
<tr >
<td> <label style="margin-left:0;"><b>Customer Signature</b></label></td> <td></td><td> <label style="float:right;"><b>Staff Signature</b></label></td>
</tr>
