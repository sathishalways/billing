 ...
 if( req_id == "" )
   {
	 ...
	 document.getElementById("reqid_error").innerHTML = "Enter the Request ID.";
     return false;
   }
   if( cust_name == "" )
   {
   ...
   if( br_name == "" )
   {
	 ...
	 document.getElementById("branch_error").innerHTML = "Enter the Branch Name";
     return false;
   }
   if( mobile == "" )
   {
     document.frmService.mobile.focus() ;
	 document.getElementById("mobile_error").innerHTML = "Enter the Mobile";
     return false;
   }
    if( service_type == "" )
   {
     document.frmService.service_type.focus() ;
	 document.getElementById("servicetype_error").innerHTML = "Select the Service_type";
     return false;
   }
   ...
   if( modelid == "" )
   {
	 ...
	 document.getElementById("model_error").innerHTML = "Select the Model";
     return false;
   }
   /* if( email_id == "" )
   {
   ...
   if(document.getElementById("serialno").value==''){
		...
		document.getElementById("serial_error").innerHTML = "Select the Serial No.";
		return false;
   }
   ...
   { 
	 ...
	 document.getElementById("assign_to_error").innerHTML = "Select the Engineer";
     return false;
   }
   /* if( serial_no == "" )
   {
...
<script>
  ...
  if ( $( "#br_name" ).val() === "" ) {
	$( "#branch_error" ).text( "Enter the Branch Name" ).show();
    event.preventDefault();
  }
  if ( $( "#mobile" ).val() === "" ) {
  ...
  if ( $( "#datetimepicker7" ).val() === "" ) {
	$( "#req_date_error" ).text( "Select the Requested Date" ).show();
    event.preventDefault();
  }
  if ( $( "#serialno" ).val() === "" ) {
	$( "#serial_error" ).text( "Select the Serial No." ).show();
    event.preventDefault();
  }
    if ( $( "#service_type" ).val() === "" ) {
	$( "#servicetype_error" ).text( "Select the Service Type" ).show();
    event.preventDefault();
  }
     if ( $( "#modelid" ).val() === "" ) {
	$( "#model_error" ).text( "Select the Model" ).show();
    event.preventDefault();
  }
      /*  if ( $( "#assign_to" ).val() === "" ) {
	$( "#assign_to_error" ).text( "Select the Engineer" ).show();
    event.preventDefault();
  } */
  if ($("#assign_to").val() == "" || $("#assign_to").val() ==null){

		$("#assign_to_error" ).text( "Select the Assign To" ).show();
		event.preventDefault();
  }
...
<script>
     $(document).ready(function(){
		...
		$("#service_type").click(function(){
			if($(this).val()==""){
			    $("#servicetype_error").show();
			}
			else{
				$("#servicetype_error").hide();
			}
		});
		$("#modelid").change(function(){
			if($(this).val()==""){
		...
		$("#assign_to").change(function(){
			if($(this).val()==""){
			    $("#assign_to_error").show();
			}
			else{
				$("#assign_to_error").hide();
			}
		});
...
<div id="reqid_error" style="color:red;font-size: 10px;"></div>
</div>
...
<?php foreach($customerlist as $customerkey){
				  if($customerkey->id==$key->cust_id){
			?>
<input type="text" value="<?php echo $customerkey->company_name.' - ('.$customerkey->customer_id.')'; ?>" readonly/>
<input type="hidden" value="<?php echo $customerkey->id; ?>" name="cust_name">
<?php } } ?>
<!--<select name="cust_name" id="cust_name" data-placeholder="Choose a Name..." class="chosen-select" tabindex="2" readonly>
			<?php foreach($customerlist as $customerkey){
				  if($customerkey->id==$key->cust_id){
			?>
           <option selected="selected" value="<?php echo $customerkey->id; ?>"><?php echo $customerkey->company_name.' - ('.$customerkey->customer_id.')'; ?></option>
...
<div id="branch_error" style="color:red;font-size: 10px;"></div>
</div>

<div class="col-md-3 col-sm-6 col-xs-12">
<label>Mobile No&nbsp;<span style="color:red;">*</span></label>
<input type="text" name="mobile" class="" id="mobile" value="<?php echo $key->mobile; ?>" readonly>
<div id="mobile_error" style="color:red;font-size: 10px;"></div>
</div>
</div>
...
<input type="text" name="email_id" id="email_id" class="" value="<?php echo $key->email_id; ?>" readonly>
</div>

<div class="col-md-3 col-sm-6 col-xs-12">
<label>Address</label>
<input type="text" name="address" id="address" value="<?php echo $key->cust_addr;?>" readonly>
</div>

<div class="col-md-3 col-sm-6 col-xs-12">
<label>Address1</label>
<input type="text" name="address1" id="address1" value="<?php echo $key->address1;?>" readonly>
</div>
...
<div id="req_date_error" style="color:red;font-size:10px;margin-top:-16px;"></div>
</div>
</div>
...
<option value="soft_up" <?php if(isset($key1->service_type) && $key1->service_type=="soft_up"){?> selected="selected" <?php } ?>>Software Upgradation</option>
<option value="Reverification" <?php if(isset($key1->service_type) && $key1->service_type=="Reverification"){?> selected="selected" <?php } ?>>Reverification</option>
<option value="Application_issue" <?php if(isset($key1->service_type) && $key1->service_type=="Application_issue"){?> selected="selected" <?php } ?>>Application Issue</option>
<option value="RoutineVisit" <?php if(isset($key1->service_type) && $key1->service_type=="RoutineVisit"){?> selected="selected" <?php } ?>>Routine Visit</option>
<option value="To_hand_over_reagent_quote" <?php if(isset($key1->service_type) && $key1->service_type=="To_hand_over_reagent_quote"){?> selected="selected" <?php } ?>>To Hand Over Reagent/Quote</option>
<option value="Reinstallation" <?php if(isset($key1->service_type) && $key1->service_type=="Reinstallation"){?> selected="selected" <?php } ?>>Reinstallation</option>
<option value="To_meet_hod" <?php if(isset($key1->service_type) && $key1->service_type=="To_meet_hod"){?> selected="selected" <?php } ?>>To Meet HOD</option>

<option value="Troubleshooting" <?php if(isset($key1->service_type) && $key1->service_type=="Troubleshooting"){?> selected="selected" <?php } ?>>Troubleshooting</option>
<option value="Training" <?php if(isset($key1->service_type) && $key1->service_type=="Training"){?> selected="selected" <?php } ?>>Training</option>

<option value="Validation" <?php if(isset($key1->service_type) && $key1->service_type=="Validation"){?> selected="selected" <?php } ?>>Validation</option>

<option value="Weekly_maintenance" <?php if(isset($key1->service_type) && $key1->service_type=="Weekly_maintenance"){?> selected="selected" <?php } ?>>Weekly Maintenance</option>

<option value="LIS_connection_printerconnection" <?php if(isset($key1->service_type) && $key1->service_type=="LIS_connection_printerconnection"){?> selected="selected" <?php } ?>>LIS Connection/Printer Connection</option>

</select>
<div id="servicetype_error" style="color:red;font-size: 10px;"></div>
</div>
...
<label>Model&nbsp;<span style="color:red;">*</span></label>
 <select name="modelid" id="modelid">
  ...
  <div id="model_error" style="color:red;font-size: 10px;"></div>
</div>
...
<select name="serial_no" id="serialno">
  ...
  <div id="serial_error" style="color:red;font-size: 10px;"></div>
</div>
...
<div class="col-md-3 col-sm-6 col-xs-12">
<label>Zone</label>
<input type="text" name="zone" id="zone" class="" value="<?php if(isset($key1->service_loc)){ echo $key1->service_loc; } ?>" readonly><input type="hidden" name="locid" id="locid" class="" value="<?php if(isset($key1->locid)){ echo $key1->locid;} ?>"><input type="hidden" name="service_loc_coverage" id="service_loc_coverage" class="" value="<?php if(isset($key1->service_loc_coverage)){ echo $key1->service_loc_coverage;} ?>">
</div>
...
<div class="ui-widget">
   <label>Problem</label>
    <input class="form-control" id="skills" name="skills" value="<?php foreach($getProbByModel as $probkey){
		if (in_array($probkey->id, $problem_data)){
			echo $probkey->prob_category.' ';
		}
...
<input type="text" name="blank_app" id="blank_app" class="" value="<?php if(isset($key1->blank_app)){ if($key1->blank_app !=0){ echo $key1->blank_app; } }?>">
</div>-->
<div class="col-md-3 col-sm-6 col-xs-12">
<label>Assign To&nbsp;<span style="color:red;">*</span></label>
<select name="assign_to[]" id="assign_to" multiple style="width:500px;" placeholder="--select--" >
			<?php foreach($employee_list as $empkey){
					/* if (preg_match("/\bStampings\b/i", $empkey->serv_category, $match)){
						$st = '1';
					}else{
					...
					//if(isset($assign_to)){
				  if(isset($assign_to) && in_array($empkey->id,explode(',',$assign_to))){
			?>
           <option selected="selected" value="<?php echo $empkey->id; ?>"><?php echo $empkey->emp_name.'- ('.$empkey->emp_id.')'; ?></option>
			<?php }  else {?>
			<option value="<?php echo $empkey->id; ?>"><?php echo $empkey->emp_name.'- ('.$empkey->emp_id.')'; ?></option>
...
<div id="assign_to_error" style="color:red;font-size: 10px;"></div>
</div>
</div>
<div class="col-md-12">
...
<button class="btn cyan waves-effect waves-light " type="submit" style="margin-right: 10px;margin-left:30px;">Submit</button>
							  <!--<a id="addRowBtn" href="<?php echo base_url(); ?>pages/add_order" class="btn cyan waves-effect waves-light" style="margin-right: 10px;">New Order</a>-->


<?php if($key1->site!='Stamping'){?>
<a onclick="print_status('<?php echo $key->id; ?>')" class="btn cyan waves-effect waves-light" style="margin-right: 10px;">Print Job sheet</a>
<?php } else{ ?>
<a onclick="print_status1('<?php echo $key->id; ?>')" class="btn cyan waves-effect waves-light">Print Stamping Job sheet</a>
...
<!--<a id="addRowBtn" href="<?php echo base_url(); ?>pages/dash" class="btn cyan waves-effect waves-light" style="margin-right: 10px;">Close</a>
<a id="addRowBtn" href="<?php echo base_url(); ?>pages/dash" class="btn cyan waves-effect waves-light">Exit</a>-->
