...
<script>
$(document).ready(function(){
$('#taxxx').change(function(){ 
	var tax_id = $(this).val();
	var tot_amt1 =$('#tot_amt1').val();
	//alert(tot_amt1);
	if(tax_id!='0'){
		var tax_amt = tot_amt1 * tax_id / 100;
		var tot = parseInt(tot_amt1) + parseInt(tax_amt);
		$('#tot_amt').val(tot);
	}else{
		$('#tot_amt').val(tot_amt1);
	}
...
<section id="content">
	...
	<?php foreach($servicereceipt as $key)
{?>
	...
	<p><b>Date : <?php echo $key->engineer_date;?></b></p>
                 ...
                 <div>
                      <form action="<?php echo base_url(); ?>spare/sparereceipt_req" method="post">
...
<label style="color: black;
    font-size: 18px;
    font-weight: bold; margin-bottom:20px;margin-top:20px;text-align:center; width:100%;" class="detai">Spares  Receipt</label>
	
	 <tr>
	 <td colspan=1 style="width: 619px;">
                <div class="pull-left" style="margin-left:30px">Receipt No :  <?php echo $key->id;?></div>
				
            </td>
			
            <td colspan=1 style="width: 619px;">
                <div class="pull-right" style="margin-right:30px">Request Id No :  <?php echo $key->request_id;?></div>
            </td>
			<input type="hidden" value="<?php echo $key->id;?>" name="spareenggis">
		...
		<tr>
			...
			<td>
				...
				<select name="taxxx" id="taxxx">
					<option value="0">---Select---</option>
					<?php foreach($get_tax as $taxKey){?>
						<option value="<?php echo $taxKey->tax_percentage; ?>"><?php echo $taxKey->tax_name.'-'.$taxKey->tax_percentage; ?></option>
					<?php } ?>	
				</select>
			</td>
		...
		<tr>
			...
			<td>
			    <input type="text" name="tot_amt" id="tot_amt" value="<?php echo $tot = $key->qty_out * $key->sales_price; ?>">
				<input type="hidden" name="tot_amt1" id="tot_amt1" value="<?php echo $tot = $key->qty_out * $key->sales_price; ?>">
			</td>
...
<a href="<?php echo base_url(); ?>spare/sparereceipt_req/<?php echo $key->id; ?>" ><input type="submit" name="submit" value="Print Job sheet" style="background: rgb(136, 104, 160) none repeat scroll 0% 0%;
    padding: 8px;
    border: none;
    color: #FFF;
    border-radius: 4px;
    margin-right: 20px;
   "></a>
<!--<a href="<?php echo base_url(); ?>spare/sparereceipt_req/<?php echo $key->id; ?>" onclick="print_status('<?php echo $key->id; ?>')" style="background: rgb(5, 94, 135) none repeat scroll 0% 0%;
padding: 11px;
color: #FFF;
border-radius: 5px;
margin-right: 20px;">Print Job sheet</a>-->&nbsp;<a class="btn cyan waves-effect waves-light " onclick="history.go(-1);">Exit
                              </a>
