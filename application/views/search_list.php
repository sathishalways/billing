...
<!--<button type="button" class="btn btn-primary btnstyle"><img src="<?php echo base_url(); ?>assets/images/addsale.png" title="Add Sales" style="width:24px;height:24px;">Add Sales</button>-->
		  ...
		  <!--<input type="text" name="search" id="search" style="position: relative; bottom: 27px;left: 78%;"/>-->
            <hr>
              ...
              <div class="row">
               ...
               <div class="col-md-12">
                  <table id="data-table-simple" class="responsive-table display" cellspacing="0">
                    <thead><?php foreach($orderlist1 as $key1){ foreach($orderlist as $key){
                        ...
                        <tr>
							...
							<th style="width:3% !important">Serial_NO</th>
							 <th style="width:7% !important">Sale Date</th>
							 <?php if($key->warranty_date != ''){ ?>
                            <th style="width:9% !important">Warranty Date</th>
							<?php } ?>
							<?php if($key->amc_end_date != ''){ ?>
                            <th style="width:9% !important">AMC Date</th>
							<?php } ?>
							<?php if($key->cmc_end_date != ''){ ?>
                            <th style="width:9% !important">CMC Date</th>
							<?php } ?>
							<?php if($key->rent_date != ''){ ?>
                            <th style="width:9% !important">Rent Date</th>
							<?php }  ?>
							<?php if($key->warranty_date!="" ||$key->rent_type!=""  ){?>
						...
						<td class="hidden"><?php echo $key1->id; ?></td>
                            ...
                            <td style="width:120px;text-align:left;" >
								...
								<?php foreach($orderlist as $key){
									if($key->order_id==$key1->id){
									?>
								<li><?php if($key->amc_end_date!=""){echo date("d-m-Y", strtotime($key->amc_end_date));} ?></li>
								<?php } } ?>
                            ...
                            <td style="width:120px;text-align:left;" >
								...
								<?php foreach($orderlist as $key){
									if($key->order_id==$key1->id){
									?>
								<li><?php if($key->cmc_end_date!=""){echo date("d-m-Y", strtotime($key->cmc_end_date));} ?></li>
								<?php } } ?>
                            ...
                            <td style="width:120px;text-align:left;" >
								...
								<?php foreach($orderlist as $key){
									if($key->order_id==$key1->id){
									?>
								<li><?php if($key->rent_date!=""){echo date("d-m-Y", strtotime($key->rent_date));} ?></li>
								<?php } } ?>
							...
							<td style="width:143px;">
								...
								<li><?php if($key->warranty_date!=""){
											echo "Warranty";
										  }elseif($key->rent_type=='syndicate')
										  {
											  echo "Syndicate";
										  }elseif($key->rent_type=='distributor')
										  {
											  echo "Local Distributor";
										  }elseif($key->rent_type=='chargeble')
										  {
											  echo "Chargeable";
										  }
