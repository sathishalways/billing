<?php 
class quote_approved_model extends CI_Model{
function __construct(){
parent::__construct();
$this->load->database();
}

	public function quote_approved_list($user_access,$user_type){
		if($user_access=="nonstamping_user"){
			...
			where (service_request.status IN(7) and (service_request_details.site='Service Center' or service_request_details.site='Customer Site')) ORDER BY service_request.id ASC");
			//echo "<pre>";print_r($query->result());exit;
			return $query->result();
	...
	public function service_req_listforProb(){
		$this->db->select("service_request_details.request_id, problem_category.prob_category",FALSE);
    $this->db->from('service_request_details');
   ...
   // $this->db->join('quote_review_spare_details', 'quote_review_spare_details.request_id = quote_review.req_id'); 
	...
	public function getQuoteReviewDetByID1($id){
		$this->db->select("quote_review.spare_tax, quote_review.spare_tot, quote_review.labourcharge, quote_review.concharge, quote_review.total_amt, quote_review.disc_amt, quote_review.delivery_date, quote_review.model_id, quote_review.notes",FALSE);
    $this->db->from('quote_review');
   // $this->db->join('quote_review_spare_details', 'quote_review_spare_details.request_id = quote_review.req_id'); 
	$this->db->where('quote_review.req_id', $id);
     	$query = $this->db->get();
	...
	public function getQuoteByReqId($id){
		$this->db->select("customers.customer_name, customers.mobile As cust_mobile , customers.company_name, customer_service_location.branch_name, customer_service_location.mobile, customer_service_location.address, customer_service_location.address1, products.model, products.id As modelid, service_request_details.warranty_date, service_request.request_id, service_request.request_date, service_request_details.site, service_request_details.machine_status, service_request_details.notes, service_request_details.blank_app, service_request_details.service_cat, service_request_details.problem, service_location.service_loc, service_location.concharge ,order_details.purchase_date,brands.brand_name",FALSE);
    $this->db->from('service_request_details');
