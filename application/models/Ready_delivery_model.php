<?php 
class ready_delivery_model extends CI_Model{
function __construct(){
parent::__construct();
$this->load->database();
}

	public function ready_delivery_list($user_access,$user_type){
		if($user_type=="1"){
			...
			$this->db->where_in('service_request.status', array('3'));
   ...
   // $this->db->join('quote_review_spare_details', 'quote_review_spare_details.request_id = quote_review.req_id'); 
	...
	public function getQuoteByReqId($id){
		$this->db->select("customers.customer_name, customers.mobile, customers.company_name, customer_service_location.branch_name, customer_service_location.mobile As cust_mobile, customer_service_location.address, customer_service_location.address1, products.model, products.id As modelid, service_request_details.warranty_date, service_request.request_id, service_request.request_date, service_request.id As req_id, service_request_details.machine_status, service_request_details.notes, service_request_details.assign_to, service_request_details.blank_app, service_request_details.service_cat, service_request_details.problem, service_request_details.accepted_engg_id, service_location.service_loc, service_location.concharge ,order_details.purchase_date, order_details.serial_no, service_request.status",FALSE);
    $this->db->from('service_request_details');
    $this->db->join('service_request', 'service_request.id = service_request_details.request_id'); 
	$this->db->join('customers', 'customers.id = service_request.cust_name'); 
