...
$this->load->database();
	...
	public function delivery_list($user_access,$user_type){
		if($user_type=="1"){
			$this->db->select("service_request.id, service_request.request_id, service_request.active_req, service_request.request_date, customers.customer_name, quote_review.delivery_date",FALSE);
			...
			$this->db->where_in('service_request.status', array('4'));
	...
	public function status_listForquoteInpro(){
		...
		$this->db->from('status'); 
		$this->db->where_in('id', array('3'));
		$query = $this->db->get();
        return $query->result();
    }
	
	public function status_listForreadydel(){
		$this->db->select("id, status",FALSE);
		$this->db->from('status'); 
   ...
   // $this->db->join('quote_review_spare_details', 'quote_review_spare_details.request_id = quote_review.req_id'); 
	...
	public function getQuoteReviewDetByID1($id){
		$this->db->select("quote_review.spare_tax, quote_review.spare_tot, quote_review.labourcharge, quote_review.concharge, quote_review.total_amt, quote_review.delivery_date, quote_review.model_id, quote_review.comments, quote_review.notes, quote_review.cust_feed, quote_review.disc_amt, quote_review.pending_amt, quote_review.cmr_paid, quote_review.payment_mode, quote_review.plus_amt, quote_review.emp_pts",FALSE);
    $this->db->from('quote_review');
   // $this->db->join('quote_review_spare_details', 'quote_review_spare_details.request_id = quote_review.req_id'); 
	$this->db->where('quote_review.req_id', $id);
     	$query = $this->db->get();
	...
	public function getQuoteByReqId($id){
		$this->db->select("customers.customer_name, customers.company_name, customer_service_location.branch_name, customer_service_location.mobile As cust_mobile, customer_service_location.address, customer_service_location.address1, products.model, products.id As modelid, service_request_details.warranty_date, service_request.request_id, service_request.request_date, service_request_details.machine_status, service_request_details.notes, service_request_details.assign_to, service_request_details.service_cat, service_request_details.problem, service_location.service_loc, service_location.concharge ,order_details.purchase_date",FALSE);
    $this->db->from('service_request_details');
    $this->db->join('service_request', 'service_request.id = service_request_details.request_id'); 
	$this->db->join('customers', 'customers.id = service_request.cust_name'); 
