...
<section id="content">
        ...
        <div class="container-fluid">
          <div class="section">
            ...
            <h5 class="breadcrumbs-title">Completed by Engineer</h5>
...
<div class="tableadd col-md-12">
	<?php if($key->status!='16' && $key->eng_ack!='accept'){ ?>
	<div class="col-md-12" style="padding-left:30px;padding-bottom:10px">
		...
		<div class="col-md-1"><input type="radio" name="eng_ack" value="accept" <?php if($key->eng_ack=='accept' && $key->eng_ack!=''){?>checked='checked'<?php }?> >Accept</div>
		<div class="col-md-1"><input type="radio" name="eng_ack" value="reject" <?php if($key->eng_ack=='reject' && $key->eng_ack!=''){?>checked='checked'<?php }?>>Reject</div>
		<div class="col-md-1"><input type="radio" name="eng_ack" value="later" <?php if($key->eng_ack=='later' && $key->eng_ack!=''){?>checked='checked'<?php }?>>Later</div>
	</div>
	<?php } ?>
	...
	<div class="col-md-12">	
		...
		<div class="col-md-3">
			...
			<input type="text" name="purchase_date" class="date" value="<?php echo $key->purchase_date; ?>" readonly>
		</div>
...
<div class="table-responsive">
 <table id="table11" class="tableadd2" style="margin-bottom: 20px;">
  ...
  <?php if($key->accepted_engg_id==$emp_id){?>
  <td style="width:50px;background: none;border: 0px"><a id="addMoreRows" style="padding: 10px;color: #000; border-radius:5px; cursor:pointer;"><span class="fa fa-plus" style="margin-right: 0px;"></span></a></td>
  <?php } ?>
 ...
 <td><input type="text" name="spare_qty[]" id="spareqty_<?php echo $count; ?>" value="<?php echo $ReviewDetKey2->spare_qty; ?>" class="spare_qty"><input type="hidden" value="<?php if(!empty($stockspare[$count])){echo $stockspare[$count];}?>" name="spare_qty1[]" id="spare_qty1<?php echo $count; ?>" class="">
	...
	<?php } ?>	
	<div name="error_quantity" id="error_sparename"></div>
 
 </td>  
 <td><input type="text" value="<?php echo $ReviewDetKey2->amt; ?>" name="amt[]" id="amt<?php echo $count; ?>" class="amout" readonly><input type="hidden" value="<?php if(!empty($salesprice[$count])){echo $salesprice[$count];}?>" name="amt1" id="amt1<?php echo $count; ?>" class="">
 ...
 <?php if(isset($ReviewDetKey2->warranty_claim_status) && $ReviewDetKey2->warranty_claim_status=='to_claim'){?>
	...
	<?php } ?>	
 <div name="error_amount" id="error_sparename"></div>
 </td>  
...
<?php if($user_type!='7'){?>
	<div class="col-md-6">
		...
		<div class="tableadd col-md-6">
			<?php if($ReviewDetKey1->notes!=""){echo $ReviewDetKey1->notes;}else{foreach($getQuoteByReqId as $notekey){echo $notekey->notes;}} ?>
		</div>
	</div>
<?php } ?>	
	<div class="col-md-6">
	...
	<!--	<div class="col-md-6"><label>Labour Charges:</label></div>
		...
		<div class="col-md-6"><input type="text" name="concharge" id="concharge" class="" value="<?php if($WarranKey->machine_status=="Chargeable"){echo $key->concharge;}else{if($i!=0){echo $i;}} ?>" readonly></div>
	...
	<?php } ?>
		...
		<div class="col-md-5">
			<?php if($user_type=='7'){?>
			<select name="status" <?php if($key->accepted_engg_id!=$emp_id){?>disabled<?php } ?>>
...
?>
	...
	<div class="col-md-3">
		<?php if($user_type=='7')
		{?>
	<select name="reporttyt" id="notes_alle" <?php if($key->accepted_engg_id!=$emp_id){?>disabled<?php } ?>>
		<option value="eng_notess"<?php if(($ReviewDetKey1->eng_notess!='') && ($ReviewDetKey1->cust_solution=='')){?>selected='selected'<?php } ?>>		
		Engineer Notes</option>
		<option value="cust_solution"<?php if(($ReviewDetKey1->eng_notess=='') && ($ReviewDetKey1->cust_solution!='')){?>selected='selected'<?php } ?>>
		Engineer Solution</option>
	</select>
		<?php
		}
		?>
	...
	<div class="eng_notess box">
	<div class="tableadd col-md-3"><textarea type="text" name="eng_notess" id="eng_notess" cols="25" rows="3" class="materialize-textarea">
	<?php if($ReviewDetKey1->eng_notess!=""){echo $ReviewDetKey1->eng_notess;} ?></textarea>
	</div>
	</div>

	<div class="cust_solution box">
	<div class="tableadd col-md-3"><textarea type="text" name="cust_solution" id="cust_solution" cols="25" rows="3" class="materialize-textarea">
	<?php if($ReviewDetKey1->cust_solution!=""){echo $ReviewDetKey1->cust_solution;} ?></textarea>
	</div>	
</div>
</div>	
</div>
...
<div class="col-md-3">
	<label>Engineer Notes & Solution:</label>
</div>
<div class="col-md-3">
<a id="<?php echo $ReviewDetKey1->req_id; ?>" href="javascript:;">Click here to view history</a>
</div>
...
<div class="col-md-3">
	<div class="tableadd col-md-3"><textarea type="text" name="cust_remarkk" id="cust_remarkk" cols="25" rows="3" class="materialize-textarea">
	<?php if($ReviewDetKey1->cust_remark!=""){echo $ReviewDetKey1->cust_remark;} ?></textarea>
</div>
...
<tr>
<td>
 <?php if($key->accepted_engg_id==$emp_id){?><button class="btn cyan waves-effect waves-light " type="submit" name="action" id="resgister">Submit
                               <i class="fa fa-arrow-right"></i>

                              </button>&nbsp;<?php } ?><a class="btn cyan waves-effect waves-light " onclick="history.go(-1);">Exit
                              </a>
</td>
</tr>
...
<script>
	$(document).ready(function(){
		...
		$("#resgister").click(function(event){
			//alert("test");
			if($("#sparename_<?php echo $count; ?>").find("option:selected").attr("value")==0){
				alert("inside if");
				$(".spare-error<?php echo $count; ?>").text("Please choose any one spare").show().css({
					'font-size':'12px',
					'color':'#ff0000'
				});
				event.preventDefault();
