...
//echo "<pre>";print_r($query->result());exit;
	...
	public function get_spare_to_enggByReqid($id)
	{	
		$query = $this->db->query("SELECT SUM(spare_to_engineers.qty_out) as qty_out, spares.id as sp_id, spares.spare_name, spare_to_engineers.req_id from spare_to_engineers as spare_to_engineers 
								 inner join spares as spares ON spares.id = spare_to_engineers.spare_id 
								 WHERE spare_to_engineers.req_id = $id GROUP BY spare_to_engineers.spare_id
		");
		//echo "<pre>";print_r($query->result());exit;
		return $query->result();
	}
	...
	public function addpettylog($m_data)
	{
		$this->db->insert('petty_log',$m_data);
	}
	...
	public function getpetsparelist($spare_id,$emp_id)
	{
		$this->db->select('*');
	...
	public function getpettyspare($spare_id,$emp_id)
	{
		/* $q1 = $this->db->get_where('spares', array('id' => $spare_id));
		return $q1->row_array(); */
	...
	public function updatepettyspare($spare_id,$t_data)
	{
		$this->db->where('id',$spare_id);
		return $this->db->update('spares',$t_data);
	}
	
	public function addpettyspare($data)
	{
		$this->db->insert('petty_spare',$data);
		
		/* $id = $this->db->insert_id();
		
		$q = $this->db->get_where('petty_spare', array('id' => $id));
		return $q->row()->spare_id; */
	}
	
	
	public function getsparelist($spare_id)
	{
		$q1 = $this->db->get_where('spares', array('id' => $spare_id));
		return $q1->row_array();
	}
	...
	public function checksparename($user)
 {
   $this->db->select('spare_name');
		$this->db->from('spares');
		$this->db->where_in('spare_name',$user);
	...
	public function get_min_spares(){
		$query=$this->db->query("SELECT * from spares where min_qty >= spare_qty");
	...
	{
		$isd='0';
		$query = $this->db->query("SELECT GROUP_CONCAT(spares.id,'//',spares.spare_name) as spare_details, service_request.request_id, service_request.id as reid, service_request.request_date, service_request.active_req, service_request_details.accepted_engg_id as empid, customers.company_name, customers.id as cust_id, employee.emp_name from quote_review_spare_details as quote_review_spare_details 
								 ...
								 inner join spares as spares ON spares.id = quote_review_spare_details.spare_id WHERE service_request.active_req = '0' GROUP BY service_request.request_id
								 ");
	...
	public function sparepurchase_view($id){
		$this->db->select("*",FALSE);
		$this->db->from('spare_purchase_details'); 
		$this->db->where('spare_id', $id);
		$this->db->order_by('purchase_date', 'desc'); 
		$this->db->where('purchase_qty !=','0');
		$query = $this->db->get();
		//echo "<pre>";print_r($query->result());exit;
        return $query->result();
    }
