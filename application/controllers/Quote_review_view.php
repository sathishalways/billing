<?php 
class quote_review_view extends CI_Controller{
	...
	public function quote_review_display(){
		$id=$this->uri->segment(3);
		$data1['req_id'] = $id;
		
		$data1['get_status']=$this->quote_review_model->get_status($id);
		
		$data1['getQuoteReviewDetByID1']=$this->quote_review_model->getQuoteReviewDetByID1($id);
		
		if(!empty($data1['getQuoteReviewDetByID1'])){
		$modelid = $data1['getQuoteReviewDetByID1'][0]->model_id; 
		$data1['spareListByModelId1']= $this->quote_review_model->spareListByModelId($modelid);
		}
		
		$data1['getQuoteReviewSpareDetByID']=$this->quote_review_model->getQuoteReviewSpareDetByID($id);
		...
		if(!empty($data1['getQuoteReviewSpareDetByID'])){
		  foreach($data1['getQuoteReviewSpareDetByID'] as $ReviewDetKey3){
			   ...
			   $data1['spareIds'][]= $this->quote_review_model->getsparedetForEditbyid($spare_id);
			}
			
		}//echo "<pre>";print_r($data1['spareIds']);exit;
		
		$data1['getQuoteByReqId']=$this->quote_review_model->getQuoteByReqId($id);
		...
		$data1['getProbforQuoteByReqId']=$this->quote_review_model->getProbforQuoteByReqId($id);
		$data1['getServiceCatbyID']=$this->quote_review_model->getServiceCatbyID($service_service_cat, $service_modelid);
		if(empty($data1['getServiceCatbyID'])){
			$data1['getservicecharge'] = '0';
		}
		
		$data1['status_list']=$this->quote_review_model->status_list();
		$data1['status_listForquoteInpro']=$this->quote_review_model->status_listForquoteInpro();
		
		$data1['getTaxDefaultInfo']=$this->quote_review_model->getTaxDefaultInfo();
		
		$data1['getWarrantyInfo']=$this->quote_review_model->getWarrantyInfo($id);
		...
		$this->load->view('templates/header',$data1);
				$this->load->view('quote_review_status',$data1);
	...
	public function addrow(){ 
		 ...
		 $data['spareListByModelId']=$this->quote_review_model->spareListByModelId($modelid);
	...
	public function getsparedet(){ 
		 ...
		 $this->output
           ...
           ->set_output(json_encode($this->quote_review_model->getsparedetbyid($id)));
    }
	...
	public function add_quotereview(){ //echo "<pre>";print_r($_POST);exit;
		...
		$result=$this->quote_review_model->add_quotereview($data);
		
		 $data4=array(
            'status'=>$this->input->post('status')
        );
		$this->quote_review_model->update_stats($data4,$reqid);
		...
		if($reqid){
           ...
           'amt' => $amt[$i]
			...
			if(!empty($data2)){
			$this->quote_review_model->update_spare_balance($data2);
			}
			
			if(!empty($data1)){
			$this->quote_review_model->add_quotereview_spares($data1);
			}
	...
	public function edit_quotereview(){ //echo "<pre>";print_r($_POST);exit;
		...
		//print_r($data['notes']);
		//exit;
		$this->quote_review_model->update_quotereview($data,$reqid);
		...
		$this->quote_review_model->delete_quote_review($reqid);
		...
		if($reqid){
           ...
           'amt' => $amt[$i]
            ...
            echo "<script>alert('Quote Review Updated');window.location.href='".base_url()."pages/quote_review';</script>";
