<?php 
class Quotereview_model extends CI_Model{
...
$this->load->database();
   ...
   // $this->db->join('quote_review_spare_details', 'quote_review_spare_details.request_id = quote_review.req_id'); 
	...
	public function getQuoteReviewSpareDetByID($id){
		$this->db->select("quote_review_spare_details.id, quote_review_spare_details.spare_id, quote_review_spare_details.spare_qty, quote_review_spare_details.amt",FALSE);
	...
	public function getQuoteByReqId($id){
		$this->db->select("customers.customer_name, customers.company_name, customer_service_location.branch_name, customer_service_location.mobile As cust_mobile, customer_service_location.landline, customer_service_location.address, customer_service_location.address1, customer_service_location.contact_name, customer_service_location.pincode, products.model, products.id As modelid, service_request_details.warranty_date, service_request.request_id, service_request.request_date, service_request.status, service_request_details.site, service_request_details.machine_status, service_request_details.notes, service_request_details.serial_no, service_request_details.blank_app, service_request_details.service_cat, service_request_details.problem, service_location.service_loc, service_location.concharge ,order_details.purchase_date, order_details.serial_no, order_details.batch_no, order_details.id as order_row_id, brands.brand_name, zone_pincodes.area_name",FALSE);
    $this->db->from('service_request_details');
    $this->db->join('service_request', 'service_request.id = service_request_details.request_id'); 
	$this->db->join('customers', 'customers.id = service_request.cust_name'); 
	$this->db->join('customer_service_location', 'customer_service_location.id = service_request.br_name'); 
	...
	public function service_req_listforaddlEmp1($id){
		$this->db->select("service_request_details.request_id, employee.emp_name, employee.id, employee.emp_mobile",FALSE);
    $this->db->from('service_request_details');
	...
	public function status_listForquoteInpro(){
		$this->db->select("id, status",FALSE);
		$this->db->from('status'); 
		$this->db->where_in('id', array('1', '7', '8'));
		$query = $this->db->get();
        return $query->result();
    }
	...
	public function getsparedetbyid($id){
		$this->db->select("spares.id, spares.spare_name, spares.spare_qty, spares.used_spare, spares.sales_price, spares.purchase_price, spares.purchase_qty",FALSE);
		$this->db->from('spare_details');
		$this->db->join('spares', 'spares.id = spare_details.spare_id'); 
	...
	public function getWarrantyInfo($id){
		$this->db->select("id, site, machine_status",FALSE);
		$this->db->from('service_request_details');
		//$this->db->join('order_details', 'order_details.serial_no = service_request_details.serial_no'); 
		$this->db->where('request_id', $id);
     	$query = $this->db->get();
